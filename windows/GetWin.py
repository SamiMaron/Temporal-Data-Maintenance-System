...
class GetWin:
    ...
    def _row10(self):
        arr = [
            ...
            Label(self._win, text=""),
            Button(self._win, text="Get", command=self.__get),
    ...
    def __get(self):
        if self.__check_valid():
            InfoTable(self._data.get_info(self._first_name.get(), self._last_name.get(), self._lonic.get(),
                                          f"{self._day.get()}-{self._month.get()}-{self._year.get()}",
                                          f"{self._hour.get()}:{self._min.get()}",
